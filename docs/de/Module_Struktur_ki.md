...
### 7.1 Block‑Struktur und Template‑Verkettung (Wichtig für Admin‑Templates)

Das Core verwendet eine Template‑Vererbung, bei der Admin‑Templates in einer übergeordneten Layout‑Datei (Parent) eingebettet werden. Dadurch kann der sichtbare Inhalt einer Modulseite nicht immer in der direkt benachbarten Datei liegen — der relevante Block (`inner_body`) kann weiter oben in der Vererbungskette definiert sein.

Empfehlungen:
- Lege sichtbare Inhalte grundsätzlich in einen `block name="inner_body"` innerhalb deines Admin‑Templates, damit das Core diese korrekt in das übergeordnete Layout injiziert.
- Da die Parent‑Datei nicht immer die unmittelbar nächstgelegene Datei ist, schaue zur Sicherheit in existierende Module (`fremeo/blog`, `fremeo/page`, `fremeo/shop`) und kopiere das dort verwendete Muster als Beispiel.
- Ergänze optionale Blöcke wie `sidebar` oder `toolbar` nur, wenn dein Modul zusätzliche Menüeinträge oder Aktionen benötigt.

Minimalbeispiel (Admin‑Template):
...
{block name="inner_body"}
  <h1>Meine Modul‑Adminseite</h1>
  {if $D.MSG}<div class="msg">{$D.MSG}</div>{/if}
  {include file="file:system/vendor/fremeo/gallery/system/template/admin__gallery.list.tpl"}
{/block}
```

Checkliste für Entwickler:
- Verwende `inner_body` für den Hauptinhalt.
- Prüfe die Vererbungskette mit `getExtends()` bzw. vergleiche mit Referenzmodulen.
- Dokumentiere in deinem Modul‑README, welche Blöcke du nutzt und warum.
